...
@click.option("--generate-images", "-i", is_flag=True, help="Generate AI profile images")
@click.option("--campaign-config", "-c", help="Path to campaign.yaml for image generation context")
def generate(app_name, description, github_url, website_url, location, project_path, platform, style, export, variations, generate_images, campaign_config):
    ...
    # Load campaign config for image generation
    campaign_data = {}
    if generate_images or campaign_config:
        if campaign_config and Path(campaign_config).exists():
            try:
                import yaml
                ...
                print_warning(f"Could not load campaign config: {e}")
        elif Path("campaign.yaml").exists():
            try:
                import yaml
                with open("campaign.yaml", 'r') as f:
                    campaign_data = yaml.safe_load(f)
                console.print("📄 Found and loaded campaign.yaml")
            except Exception as e:
                print_warning(f"Could not load campaign.yaml: {e}")
        
        # Use project info as fallback for campaign data
        if not campaign_data:
            campaign_data = {
                "name": project_info.get("name", "App"),
                "concept": project_info.get("description", "Software application")
            }
    
    if generate_images:
        console.print(f"\n🎨 Generating profiles with AI images for {len(selected_platforms)} platforms...")
    else:
            ...
            try:
                if variations:
                    ...
                    for i, var_profile in enumerate(variations_list):
                        ...
                        # Add AI images if requested
                        if generate_images:
                            var_profile = await generator.generate_profile_images(
                                var_profile, campaign_data, True
                            )
                    ...
                    if generate_images:
                        # Use the new async method with image generation
                        profile_content = await generator.generate_profile_with_images(
                            plat, project_info, campaign_data, style, True
                        )
                    else:
                        # Use the regular sync method
    ...
    if generate_images:
        ...
        # Run sync version for non-image generation
        for plat in selected_platforms:
            try:
                if variations:
                    # Generate multiple variations
...
def _show_detailed_profile(profile: ProfileContent) -> None:
    ...
    # Show profile image info (both suggestion and generated)
    if hasattr(profile, 'profile_image_path') and profile.profile_image_path:
        content_parts.append(f"[bold]Profile Image:[/bold] ✅ Generated → [cyan]{profile.profile_image_path}[/cyan]")
    else:
        content_parts.append(f"[bold]Profile Image:[/bold] {profile.profile_image_suggestion}")
    
    # Show cover image info (both suggestion and generated)
    if profile.cover_image_suggestion and profile.cover_image_suggestion != "Not applicable":
        if hasattr(profile, 'cover_image_path') and profile.cover_image_path:
            content_parts.append(f"[bold]Cover Image:[/bold] ✅ Generated → [cyan]{profile.cover_image_path}[/cyan]")
        else:
            content_parts.append(f"[bold]Cover Image:[/bold] {profile.cover_image_suggestion}")
    
    # Show AI generated images summary
    if hasattr(profile, 'generated_images') and profile.generated_images:
        content_parts.append(f"[bold]AI Generated Images:[/bold] {len(profile.generated_images)} files created")
